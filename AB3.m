...
% Método de Adams-Bashforth de tres pasos para resolver un sistema de EDOs 
...
% Primeras dos iteraciones con el método de Runke-Kutta
for k=1:2
    k1 = f(t(k), y(k,:));
...
for k=3:n
    y(k+1,:) = y(k,:) + (h/12)*(23*f(t(k),y(k,:))-16*f(t(k-1),y(k-1,:))+...
        5*f(t(k-2),y(k-2,:)));
end
